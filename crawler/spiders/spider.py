...
class NewsRAGSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        def is_valid_author(name):
            ...
            if name_lower in ['nguon', 'theo', 'anh', 'bai', 'tac gia', 'tong hop', 'pv', 'btv']:
                return False
            ...
            bad_words = [
                ...
                'ngay', 'thang', 'nam', 'phut truoc', 'gio truoc',
                'anh:', 'video:', 'xem them', 'ban quyen', 'chia se', 'lien he', 'hotline'
            ]
        ...
        fake_authors = [
            'vietnamnet news', 'vietnamnet', 'ban bien tap',
            'giam ngheo benh vuong', 'dan tri', 'thoi su', 'kinh te'
        ]
        ...
        if not author:
            ...
            for a in author_selectors:
                ...
                if a and isinstance(a, str) and a.strip() and a.strip() != "Unknown":
                    ...
                    clean_a = re.sub(r'(?i)^(?:Nguon|Theo|Source|Anh va bai|Bai va anh|Anh|Tong hop)\s*:?\s*', '', clean_a).strip()
                    if is_valid_author(clean_a):
                        author = clean_a
                        break

        if not author or author == "Unknown":
            main_content = response.css('div[class*="content"], article, div[id*="content"], .post-body, .detail-content')
            target_area = main_content if main_content else response
            bottom_nodes = target_area.xpath('.//p | .//div[contains(@class, "author") or contains(@class, "right") or contains(@style, "right")]')
            ...
            if bottom_nodes:
                for node in reversed(bottom_nodes[-15:]):
                    ...
                    if len(clean_text) < 2 or len(clean_text) > 100:
                        continue
                    match_source = re.search(r'(?i)(?:Nguon|Theo|Source)\s*:?\s*(.+)', clean_text)
                    if match_source:
